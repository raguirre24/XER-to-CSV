...
`free_float_status`, `free_float_basis`, `classification`, `reason_code` and `message` explain each result. `Finite` and `Estimated` can carry a signed number; resource-dependent activity endpoints retain a labelled task-calendar estimate. `Historical`, `FixedEvent`, `NoFiniteBound`, `RequiresContext`, `MissingData` and `InvalidData` remain nonnumeric. Blank is not zero and must not be treated as a driving relationship. The legacy classification remains available separately; successful publication does not mean all relationships have a numeric result.

The fixed 46-column schema (`audit_schema_version=1.1`) is declared in [RelationshipAuditCsv.cs](../XerToCsvConverter.Core/RelationshipAuditCsv.cs). Version 1.1 inserts `free_float_status` and `free_float_basis` immediately after `free_float`; the existing `reason_code` is retained. It includes public endpoint identities, statuses, mode, SS lag basis, raw/effective lag, required calendars, selected endpoint timestamps and source fields. Six successor scheduling options have separate raw/state columns. `input_evidence` is a JSON object containing all captured raw fields and presence/validity states for both activities, projects, scheduling contexts and the relationship. Import evidence and raw date strings as text; do not coerce malformed source values into dates or zeros.
