...
$rows = for ($i = 0; $i -lt $assessments.Count; $i++) {
    ...
    foreach ($pair in @(@('free_float_status', $a.AllowanceStatus.ToString()), @('free_float_reason', $a.ReasonCode), @('free_float_basis', $a.CalculationBasis.ToString()))) {
        if ($table06[$i].($pair[0]) -cne $pair[1]) { throw "Table 06 $($pair[0]) disagrees with assessment at row $($i+1)." }
    }
    ...
    [pscustomobject]@{
        ...
        allowance_status = $a.AllowanceStatus.ToString(); calculation_basis = $a.CalculationBasis.ToString()
    }
}
$rows | Export-Csv -LiteralPath (Join-Path $PSScriptRoot 'real-sample-relationships.csv') -NoTypeInformation -Encoding utf8
$matrix = $rows | Group-Object relationship_type,predecessor_status,successor_status,classification,reason_code,allowance_status,calculation_basis | ForEach-Object {
    $first = $_.Group[0]
    [pscustomobject]@{
        ...
        allowance_status = $first.allowance_status; calculation_basis = $first.calculation_basis
    }
...
$summary = [ordered]@{
    ...
    allowance_statuses = @($rows | Group-Object allowance_status | ForEach-Object { @{allowance_status=$_.Name; count=$_.Count} })
    calculation_bases = @($rows | Group-Object calculation_basis | ForEach-Object { @{calculation_basis=$_.Name; count=$_.Count} })
