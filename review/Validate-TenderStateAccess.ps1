...
Assert-Tender ($readTable -match '(?s)ContextChecked\s*=.*InvalidProjectContext.*UniqueChecked\s*=.*ContextChecked.*WithManifestState\s*=.*Table.TransformColumns\(UniqueChecked,.*bundle\[ProjectState\].*CalendarChecked\s*=.*WithManifestState.*WithProject\s*=.*CalendarChecked.*WithSource\s*=\s*Table.AddColumn\(WithProject,.*\bin\s+WithSource' -and $readTable.Contains('CalendarChecked = if tableName = "11_XER_CALENDAR_DETAILED" then fnXerCsvValidateCalendarDetails(WithManifestState, bundle) else WithManifestState') -and $readTable.Contains('WithProject = if tableName = "11_XER_CALENDAR_DETAILED" then Table.AddColumn(CalendarChecked, "ProjectKey", each bundle[ProjectCode], type nullable text) else CalendarChecked')) 'Manifest State overwrite bypasses mirror validation or is not on the returned output path.'
