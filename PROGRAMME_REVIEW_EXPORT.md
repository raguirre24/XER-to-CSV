...
- Tender no longer rewrites spaces to underscores: `NE Part B` exports as `NE PART B`, distinct from `NE_PART_B`. Regenerate all stages together when replacing an older bundle that used the underscore spelling. Tender's existing `C<digits>` / `J<digits>` aliases still apply. This naming extension does not alter numbered CSV headers or calculation schemas. The local Programme Review and Tender Review report loaders now accept only their current contracts (4.0 and 2.0 respectively), use the same encoded project filename/key components, and support JSON arrays for project lists containing commas. Deployed or other report copies need the corresponding loader update; export validation does not establish a live report refresh. For flexible names, the parent SharePoint project folder uses the encoded filename component, while routing parameters and manifest `project_code` retain the full normalised business code.
