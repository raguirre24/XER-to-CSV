...
It creates one full-history local bundle for one project and programme type. The bundle is validated and written to a staging directory first; the final directory only appears after all eleven numbered CSV files are complete, their hashes have been calculated, and `XER_CSV_MANIFEST.csv` has been written last: twelve files in total. Programme schema 5.0 and Tender schema 4.0 add detailed calendar table 11; regenerate older bundles with the matching parser/report versions.

The diagnostic companion `XER_DATA_QUALITY.csv` is reserved for Standard Enhanced exports and is not emitted in Programme Review or Tender Review bundles. Diagnostic warnings are exposed on the result object (`DataQualityTable`, `WarningCount`) and logged. Invalid actual date periods do not block otherwise valid allocations: valid remaining work still distributes. Windows, Web and CLI report completion with warnings; CLI exit code remains zero. Manifest `complete` means publication completed, not that the schedule is free of data-quality issues. The manifest contains eleven rows per retained source (one per contract table). See [the warning and reconciliation contract](review/NONBLOCKING_ACTUAL_WARNINGS.md).
...
- Both Programme and Tender accept any nonblank project code, including `QAC000623-01-02`, spaces, punctuation and Unicode. Codes are trimmed and uppercased; internal whitespace and punctuation are preserved. `programme_type` remains `C` for Contract or `T` for Target.
- Tender's entered **reporting project code** is an explicit output mapping, not an equality constraint on P6 `PROJECT.proj_short_name`. For example, one source named `QAC000623-01-02` can export under reporting code `QAC000623`. Native names are never shortened or rewritten, and no prefix-based equivalence is inferred. Each selected XER must still contain exactly one `PROJECT` row with a nonblank native ID/name and valid Data Date; native task/project references remain source-local. Selecting stages assigns them to the entered reporting project, including renamed P6 revisions. Every non-alias mapping emits `TENDER_PROJECT_CODE_MAPPED` in result diagnostics with the original short name and requested report code. Confirm that selected stages belong to the same reporting project; neither a shared prefix nor the chosen output label proves that business relationship.
- `ProjectCode` and manifest `project_code` carry the complete normalized business code. Generated canonical filenames, bundle folder/ZIP names and key project components use UTF-8 percent encoding for characters other than ASCII letters, digits, underscore, hyphen and space. Literal `%` is encoded too, so `A/B`, `A%2FB`, `A_B` and `A B` remain separate identities. Consume exported keys without decoding or recreating them. File components over 100 encoded characters use `~` plus the full SHA-256 of the normalized code; key components and metadata retain the complete identity. Existing ASCII letter/digit/underscore codes keep their previous names and keys.
- Tender no longer rewrites spaces to underscores: `NE Part B` exports as `NE PART B`, distinct from `NE_PART_B`. Regenerate all stages together when replacing an older bundle that used the underscore spelling. Tender has no C/J aliases; Programme's existing rules remain separate. Current-only report loaders accept Programme 5.0 and Tender 4.0, use the same encoded project filename/key components, and support JSON arrays for project lists containing commas. Deployed or other report copies need the corresponding loader update; export validation does not establish a live report refresh. For flexible names, the parent SharePoint project folder uses the encoded filename component, while routing parameters and manifest `project_code` retain the full normalised business code.
...
CSV files are UTF-8 without a BOM, comma-delimited, RFC-style quoted, and use CRLF records. Dates are `yyyy-MM-dd`; numeric and integer tokens use invariant culture; logical columns use lowercase `true`/`false`. Optional source tables are emitted header-only. The exact ordered schemas are exposed by `ProgrammeReviewContract.Tables` as schema version `5.0` (Tender Review uses `4.0`). Table 06 includes `free_float_status`, `free_float_basis` and `free_float_reason` immediately after `free_float`.

Table 11 has exactly these columns, including the case of `MonthUpdate`:
...
It reuses the Standard calendar decoder without changing calendar arithmetic. Seven undated weekday rules accompany dated replacement exceptions; invalid rules/markers remain visible with blank derived values. `working_day` is Y/N text, `work_hours` numeric, weekday numbers are Monday=1 to Sunday=7, and `working_day_int` is 1/0. Unknown is not zero. Review keys use their governed source namespace; `MonthUpdate` uses Programme snapshot metadata or Tender's P6 Data Date, not Tender `status_date` or a filename guess. The table is not unique on `clndr_id_key` and is not a dense Date dimension. No calendar source produces a header-only file. Raw `clndr_id` and provenance `FileName` remain Standard-only columns.
...
The current report reuses its existing `SharePointSite`, whose library is `Documents`; `XerCsvLibrary` may be changed if an override site exposes a different exact library name. Routing parameters and manifest metadata use the full normalised business code; the project folder uses its safe encoded filename component. Upload the eleven table CSVs first and `XER_CSV_MANIFEST.csv` last. Do not copy the staging directory or a partially written bundle. Manifest-free staging folders are ignored. Power BI orders contract-valid manifest-bearing folders by the timestamp in `bundle_id`, opens only the newest selected manifest, and fails refresh if that selected bundle is invalid rather than silently reverting to older data. After the report has accepted the new bundle, move superseded bundles to `Archive/<PROJECT_CODE>`. Rollback requires a matching older parser/report/bundle set, not an old-version bundle with the new loader.
