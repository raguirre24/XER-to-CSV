...
- Periods are half-open `[start, finish)` with contiguous midnight month boundaries. Positive units need valid ordered dates and nonzero working availability.
...
Named curves require fixed duration types `DT_FixedDrtn` or `DT_FixedDUR2`. Nonlinear named curves are supported for unstarted activities/assignments only; active nonlinear work needs explicit remaining profiles. A linear named curve is phase-independent and can support active work. Manual curve ID9 without `remain_crv`, nonzero index0 and opaque `RSRCCURV.curv_data`-only definitions are unsupported. Do not infer curve phase from activity percent complete. `target_crv` does not substitute for remaining units; `actual_crv` does not change the current actual spread.
