...
`distribution_month` is the monthly bucket date, independent of the source's update/status date. `Start`/`Finish` are allocation-period timestamps, not necessarily activity endpoints. `is_actual` separates actual from remaining. `distribution_type` distinguishes `Working Hours`, `Resource Curve`, `Remaining Units Profile`, `Actual Elapsed Time` and `Actual Recorded Date` in legacy; it is not included in the review projection. The last two preserve recorded actual units when the entire valid period has no scheduled work, or the timestamps coincide. Their working-hour/day diagnostics remain zero (day conversions blank if HPD is unknown); a recorded instant also has zero calendar-duration diagnostics. These labels do not claim timesheet detail or authorize fallback for remaining units.
...
This is not another numbered monthly table. It accompanies every table 15 export, including a header-only file when no recoverable actual-date issues exist. All profiles use the same 24-column header (including writer provenance):
...
Grain is one unresolved actual portion per source assignment; `diagnostic_schema_version=1.0`, `severity=Warning`, and `table_name=15_XER_RESOURCE_DISTRIBUTION`. Original assignment dates/quantity strings and raw project Data Date are preserved as text, including missing or malformed values. `status_code` is the raw TASK status. `unallocated_actual_quantity` is known regular plus overtime units rounded to four decimals, not zeroed or assigned to a guessed month. `message` explains the issue; `issue_code` gives a stable classification. Dates in this companion are intentionally not the date-normalized review columns.

The four keys use the same public namespace as that profile's numbered tables; `taskrsrc_id_key` qualifies the native assignment ID when present. `source_namespace` is the Standard collision-qualified filename namespace or governed review namespace without its trailing `::`. It is not the internal token. Internal occurrence tokens still protect all correlation before serialization. `FileName` preserves the original name, including repeats. `source_row_number` is the one-based TASKRSRC row ordinal within that occurrence, providing traceability when an assignment ID is absent; it is not a durable cross-export business key. Keep bundle/source provenance when comparing warnings across exports.

Use this file to show unallocated actuals and source-quality problems. Do not add its units to a monthly trend as though a month were known. Reconcile distributed plus unallocated actuals at compatible source/task/resource grain; keep valid remaining allocations from the same assignment. The review manifest includes per-source companion row counts and its CSV hash; completion literals indicate completed publication, while a nonzero companion count means completed with warnings.
