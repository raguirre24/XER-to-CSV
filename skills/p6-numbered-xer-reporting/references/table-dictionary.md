...
An optional relationship assessment explains each raw TASKPRED row separately, including calculated, ignored, historical, unsupported, missing-data and invalid-data outcomes. Its selected remaining endpoints can differ from the displayed Start/Finish fields. The audit is not an extra numbered table and does not extend the normal file/manifest envelope. Use [relationship-audit.md](relationship-audit.md) when inspecting those reasons or raw field states.
